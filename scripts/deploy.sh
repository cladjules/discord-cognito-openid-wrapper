...
sam deploy --region "$REGION" --template-file "$OUTPUT_TEMPLATE_FILE" --stack-name "$STACK_NAME" --parameter-overrides DiscordClientIdParameter="$OAUTH_CLIENT_ID" DiscordClientSecretParameter="$OAUTH_CLIENT_SECRET" ProviderName="$PROVIDER_NAME" CognitoRedirectUriParameter="$COGNITO_REDIRECT_URI" StageNameParameter="$STAGE_NAME" --capabilities CAPABILITY_IAM
